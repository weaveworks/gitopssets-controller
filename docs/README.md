...
The `repeat` field can be either a [JSONPath](https://kubernetes.io/docs/reference/kubectl/jsonpath/) expression that is applied to each element during the template rendering, or alternatively a [CEL](https://kubernetes.io/docs/reference/using-api/cel/) expression.  CEL expressions must be prefixed with the string `cel:`.

The JSONPath expression above indicates that the `ConfigMap` should be generated for each of the `teams`, this can be expressed in CEL as `cel:Element.teams`.
